...
public class PLManager implements PLIView, SensorEventListener, OnDoubleTapListener {
		...
		if(!mIsValidForTransition) {
			...
			if(panorama != null)
		    {
	            ...
	            else
	            {
	            	...
	            	mPanorama = panorama;
					activity.setContentView(this.onGLSurfaceViewCreated(mGLSurfaceView , getResolution()));
	            }
		    }
		    else
		    {
	    	...
	    	if(showProgressBar)
	    	{
	    		...
	    		(
	    			new Runnable()
	    			{
						...
						public void run()
						{
							loader.load(PLManager.this, transition, initialPitch, initialYaw);
						}
	    			},
	    			300
	    		);
     ...
     */
	@SuppressWarnings("deprecation")
	protected View onGLSurfaceViewCreated(GLSurfaceView glSurfaceView , int height) {

		for(int i = 0; i < kMaxTouches; i++)
			mInternalTouches.add(new UITouch(glSurfaceView, new CGPoint(0.0f, 0.0f)));
		mContentLayout = new RelativeLayout(activity);
		mContentLayout.setLayoutParams(new LayoutParams(LayoutParams.FILL_PARENT, height));
		mContentLayout.addView(glSurfaceView, new LayoutParams(LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
